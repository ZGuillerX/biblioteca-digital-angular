...
export class GoogleBooksSearchComponent implements OnInit, OnDestroy {
  ...
  async handleSearch(): Promise<void> {
    ...
    try {
      ...
      this.results = books.filter((b) => {
        ...
        if (this.searchField === "title") return (b.title || "").toLowerCase().includes(q)
        if (this.searchField === "description") return (b.description || "").toLowerCase().includes(q)
        if (this.searchField === "category") return (b.category || "").toLowerCase().includes(q)
        return true
      })
    } catch (err: any) {
  ...
  async handleAddBook(googleBook: GoogleBook): Promise<void> {
    try {
      ...
      console.log("[v0] Agregando libro desde Google Books:", {
      ...
      console.log("[v0] Datos del libro a crear:", newBook)

      const created = await this.bookService.create(newBook)

      console.log("[v0] Libro creado:", created)
